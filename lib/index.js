        ...
        visitor: {
            CallExpression (path) {
                ...
                const catchPath = path.findParent(({ node }) => {
                    return t.isFunction(node) || isObjectProperty(t, node.callee, 'catch')
                })
                ...
                const mostOuterThenPath = path.findParent(pPath => {
                    ...
                    return t.isFunction(node) || (isObjectProperty(t, node.callee, 'then') && !t.isMemberExpression(pPath.parentPath.node))
                })
